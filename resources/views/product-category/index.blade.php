        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="bg-white border-b border-gray-200 mt-3 lao-font py-8">
                    <h4 class="text-info"><i class="fa fa-object-group" aria-hidden="true"></i> ໝວດສິນຄ້າ</h4>

                    <div class="row">
                        <div class="col-md-4">
                            <form method="POST" action="{{ route('pro-category.store') }}" id="form-cate">
                                @csrf
                                {{-- ເລືອກກຸ່ມສິນຄ້າມາຈາກຖານຂໍ້ມູນ sml --}}
                                <label for="from_base"><span class="text-danger">*</span> ໝວດສິນຄ້າຈາກ
                                    SML</label>
                                ...
                                <!-- full Name -->
                                <div>
                                    <label for="pro_group"><span class="text-danger">*</span> ກຸ່ມສິນຄ້າ</label>

                                    <select name="pro_group" id="pro_group" class="form-control" required>
                                        <option value="">...</option>
                                        @foreach ($pro_group as $key => $prg_item)
                                            <option value="{{ $prg_item->id }}">{{ $prg_item->prg_name }}</option>
                                        @endforeach
                                ...
                                <!-- full Name -->
                                <div class="mt-4">
                                    <label for="prc_name"><span class="text-danger">*</span> ຊື່ໝວດສິນຄ້າ</label>

                                    <x-input id="prc_name" class="form-control" type="text" name="prc_name"
                                        :value="old('prc_name')" required autofocus placeholder="..." />
                                </div>
                                <!-- User Role -->
                                <div class="mt-4">
                                    <label for="prc_note">ໝາຍເຫດ</label>

                                    <textarea name="prc_note" id="prc_note" class="form-control" rows="5"></textarea>
                                </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="#" class="text-warning" data-toggle="modal"
                                                    ...
                                                    onclick="set_edit_pro_cate({{ $item->id }}, {{ $item->pg_id }}, '{{ $item->prc_name }}', 
                                                    '{{ $item->note }}', '{{ $item->code_fb }}', '{{ $item->from_base }}')"><i
                                                        class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                            </td>
                                            <td class="text-center">
                                                <a href="#" class="text-danger"
                                                    onclick="delete_product_cate({{ $item->id }})"><i
            ...
            <div class="modal-content">
                ...
                <div class="modal-body lao-font">
                    <form method="POST" action="" id="form-edit-prg">
                        ...
                        <!-- full Name -->
                        <div>
                            <label for="pro_group_edit"><span class="text-danger">*</span> ກຸ່ມສິນຄ້າ</label>

                            <select name="pro_group_edit" id="pro_group_edit" class="form-control" required>
                                <option value="">...</option>
                                @foreach ($pro_group as $key => $prg_item)
                                    <option value="{{ $prg_item->id }}">{{ $prg_item->prg_name }}</option>
                                @endforeach
                        ...
                        <!-- full Name -->
                        <div>
                            <label for="edit_prc_name"><span class="text-danger">*</span> ຊື່ໝວດສິນຄ້າ</label>

                            <x-input class="form-control" type="text" name="edit_prc_name"
                                :value="old('edit_prc_name')" required autofocus placeholder="ຊື່ໝວດສິນຄ້າ..." />
                        </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(e) {
                ...
                $.each(e, function(i, item) {
                    ...
                    data-dismiss="modal" onclick="get_cate_fb('${code}','${name}','${base}')">
    ...
    function get_cate_fb(code, name, base) {
        let states = $("#add-states").val();
        if (states == 'add') {
            $("#form-cate input[name='from_base']").val(base);
            $("#form-cate input[name='group_fb_code']").val(code);
            $("#form-cate input[name='group_fb_name']").val(name);
        } else {
    ...
    function set_edit_pro_cate(id, pg_id, prg_name, note, code, base) {
        $("#form-edit-prg input[name='from_base']").val(base);
        $("#form-edit-prg input[name='group_fb_code']").val(code);
        $("#form-edit-prg input[name='group_fb_name']").val('');
        //
        $('#pro_group_edit option[value=' + pg_id + ']').attr('selected', 'selected');
        $('#form-edit-prg input[name="prg_id"]').val(id);
        $('#form-edit-prg input[name="edit_prc_name"]').val(prg_name);
